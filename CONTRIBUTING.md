...
==========================
Leviathan is happy you're willing to contribute to our projects. We are usually
very lenient with all submitted PRs, but there are still some guidelines you
can follow to make the approval process go more smoothly.

## Use a Personal Fork and not an Organization

Leviathan will routinely modify your PR, whether it's a quick rebase or to take care
...
Unfortunately, if you use an organization for your PR, it prevents Leviathan from
modifying it. This requires us to manually merge your PR, resulting in us
closing the MR instead of marking it as merged.
...
- A Java 21 or later JDK (packages vary, use Google/DuckDuckGo/etc.).
    ...
    - Leviathan requires JDK 21 to build, however, makes use of Gradle's
...
- `Leviathan-API` - Modifications to `gale-api`/`Paper-API`;
- `Leviathan-Server` - Modifications to `Gale`/`Paper`.
...
3. cd into `Leviathan-Server` for server changes, and `Leviathan-API` for API changes.

`Leviathan-Server` and `Leviathan-API` aren't git repositories in the traditional sense:

- `base` points to the unmodified source before Leviathan patches have been applied.
- Each commit after `base` is a patch.

## Adding Patches

Adding patches to Leviathan is very simple:

1. Modify `Leviathan-Server` and/or `Leviathan-API` with the appropriate changes;
...
1. Run `./gradlew rebuildPatches` in the main directory to convert your commit into a new
   patch;
1. MR the generated patch file(s) back to this repository.

Your commit will be converted into a patch that you can then MR into Leviathan.
...
the `Leviathan-Server` and/or `Leviathan-API` folders.
...
> ideal nor intentional, but it happens. Feel free to fix this in a MR to us!
...
1. MR your modified patch file(s) back to this repository.
...
Steps to rebase a MR to include the latest changes from `main`.  
These steps assume the `origin` remote is your fork of this repository and `upstream` is the official Leviathan repository.

1. Pull the latest changes from upstreams main: `git checkout main && git pull upstream main`.
...
1. If your MR creates new patches instead of modifying existing ones, in both the `Leviathan-Server` and `Leviathan-API` directories, ensure your newly-created patch is the last commit by either:
...
Remember that these changes will affect everyone who runs Leviathan, not just you
and your server.
...
All modifications to non-Leviathan files should be marked. The one exception to this is
...
- You need to add a comment with a short and identifiable description of the patch:
  `// Leviathan start - <COMMIT DESCRIPTION>`
...
- Multi-line changes start with `// Leviathan start - <COMMIT DESCRIPTION>` and end
  with `// Leviathan end - <COMMIT DESCRIPTION>`.
- One-line changes should have `// Leviathan - <COMMIT DESCRIPTION>` at the end of the line.

Here's an example of how to mark changes by Leviathan:

```java
entity.getWorld().dontBeStupid(); // Leviathan - Was beStupid(), which is bad
entity.getFriends().forEach(Entity::explode);
entity.updateFriends();

// Leviathan start - Use plugin-set spawn
// entity.getWorld().explode(entity.getWorld().getSpawn());
Location spawnLocation = ((CraftWorld)entity.getWorld()).getSpawnLocation();
entity.getWorld().explode(new BlockPosition(spawnLocation.getX(), spawnLocation.getY(), spawnLocation.getZ()));
// Leviathan end - Use plugin-set spawn
```
...
public class SomeEvent extends Event {
    public final org.bukkit.Location newLocation; // Leviathan - add location
}
```

## Access Transformers
Sometimes, vanilla or CraftBukkit code already contains a field, method, or type you want to access
but the visibility is too low (e.g. a private field in an entity class). Leviathan can use access transformers
...
When submitting patches to Leviathan, we may ask you to add notes to the patch
...
without having to dig back through git history looking for your PR.
...
If you add a message to your commit in the `Leviathan-Server`/`Leviathan-API`
...
to make future updates easier on ourselves, Leviathan tries to use obfuscation
...
```java
double d0 = entity.getX(); final double fromX = d0; // Leviathan - OBFHELPER
// ...   
this.someMethod(fromX); // Leviathan
```
...
To build and install the Leviathan APIs and Server to your local Maven repository, do the following:
...
- Add `mavenLocal()` as a repository. Gradle checks repositories in the order they are declared,
  so if you also have the Leviathan repository added, put the local repository above Leviathan's.
...
- If you later need to use the Leviathan-API, you might want to remove the jar
...
By default, Leviathan (and upstream) only import files we make changes to. If you
would like to make changes to a file that isn't present in `Leviathan-Server`'s
...
1. Identify the name(s) of the file(s) you want to import.
    - A complete list of all possible file names can be found at
      `./Leviathan-Server/.gradle/caches/paperweight/mc-dev-sources/net/minecraft/`. You might find
