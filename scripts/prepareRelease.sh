...
mv ./leviathan-server/build/libs/leviathan-paperclip-${MC_VERSION}*.jar ./$JAR_NAME-${BUILD_NUMBER}.jar
...
if [ $IS_EOL = true ]; then
  {
    echo ""
    echo "> [!WARNING]"
    echo "> This version of Leviathan is end-of-life and will only receive critical bugfixes from upstream."
...
if [ $IS_UNSUPPORTED = true ]; then
  {
    echo ""
    echo "> [!CAUTION]"
    echo "> This version of Leviathan is unsupported and will not receive any bugfixes."
