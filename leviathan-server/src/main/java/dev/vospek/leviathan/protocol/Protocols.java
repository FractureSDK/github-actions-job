...
public final class Protocols {
    ...
    public record TypeAndCodec<B extends FriendlyByteBuf, T extends LeviathanCustomPayload>(LeviathanCustomPayload.Type<T> type,
                                                                                       StreamCodec<B, T> codec) {
    }

    public static <B extends FriendlyByteBuf> void write(B byteBuf, LeviathanCustomPayload payload) {
    ...
    public static void handle(ServerPlayer player, DiscardedPayload payload) {
        for (Protocol protocol : PROTOCOLS) {
            if (payload.type().id().getNamespace().equals(protocol.namespace())) {
                var leviathanCustomPayload = decode(protocol, payload);
                if (leviathanCustomPayload != null) {
                    protocol.handle(player, leviathanCustomPayload);
                }
                return;
            }
        }
    ...
    @Contract("_ -> new")
    public static ClientboundCustomPayloadPacket createPacket(LeviathanCustomPayload payload) {
        return new ClientboundCustomPayloadPacket(payload);
    }

    private static <B extends FriendlyByteBuf> void encode(B byteBuf, LeviathanCustomPayload payload, Protocol protocol) {
    ...
    private static @Nullable LeviathanCustomPayload decode(Protocol protocol, DiscardedPayload payload) {
