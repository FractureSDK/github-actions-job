...
public class FastRNG extends ConfigModule {
    ...
    public void onLoaded() {
        ...
        if (enabled) {
            try {
                Class.forName("dev.vospek.leviathan.util.math.random.FasterRandomSource");
            } catch (Throwable ignored) {
                LeviathanConfig.LOGGER.error("Faster random generator is enabled but Xoroshiro128PlusPlus is not supported by your JVM, " +
        ...
        if (enabled && warnForSlimeChunk) {
            LeviathanConfig.LOGGER.warn("You enabled faster random generator, it will offset location of slime chunk");
            LeviathanConfig.LOGGER.warn("If your server has slime farms or facilities need vanilla slime chunk,");
            LeviathanConfig.LOGGER.warn("set performance.faster-random-generator.use-legacy-random-for-slime-chunk " +
                "to true to use LegacyRandomSource for slime chunk generation.");
            LeviathanConfig.LOGGER.warn("Set performance.faster-random-generator.warn-for-slime-chunk to false to " +
