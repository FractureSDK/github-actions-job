...
import dev.vospek.leviathan.async.world.UnsafeReadPolicy;
...
public class SparklyPaperParallelWorldTicking extends ConfigModule {
    ...
    public void onLoaded() {
        ...
        if (runAsyncTasksSync != null && runAsyncTasksSync) {
            LeviathanConfig.LOGGER.warn("The setting '{}.run-async-tasks-sync' is deprecated, removed automatically. Use 'async-unsafe-read-handling: BUFFERED' for buffered reads instead.", basePath());
        }

        if (enabled) {
            LeviathanConfig.LOGGER.info("Using {} threads for Parallel World Ticking", threads);
        }
    }
}
