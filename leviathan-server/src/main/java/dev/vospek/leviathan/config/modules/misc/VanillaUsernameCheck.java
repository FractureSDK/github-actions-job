    ...
    public void onLoaded() {
        ...
        if (!regexString.isBlank()) {
            try {
                ...
                LeviathanConfig.LOGGER.error("Invalid username regex {} found, falling back to default.", regexString, e);
            }
        }
        if (useUsernameRegex && removeAllCheck) {
            LeviathanConfig.LOGGER.warn("Found conflicting configuration, remove-all-check and use-username-regex cannot be enabled at same time, ignoring remove-all-check...");
